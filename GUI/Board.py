...
class GUICell:
    ...
    def draw_cell(self, screen):
        ...
        if self.explode and self.value == "M":
            if (
                self.explode
                and pygame.time.get_ticks() - self.explosion_animation_frames < 650
            ):
                ...
                screen.blit(
                    self.explosion_frames[
                        int(
                            ...
                            * (
                                ...
                                - self.explosion_animation_frames
                            )
                            / 650
                        )
                    ],
                    ...
                    ].get_rect(center=self.rectangle.center),
                )
            else:
                screen.blit(
                    self.value_image[1],
        ...
        # Handling flag picking and idle animations
            ...
            if pygame.time.get_ticks() - self.flag_pick_animation_frames < 300:
                screen.blit(
                    self.flag_remove_framse[
                        int(
                            3
                            * (
                                pygame.time.get_ticks()
                                - self.flag_pick_animation_frames
                            )
                            / 300
                        )
                    ],
        ...
        # Handling flag placing and idle animations
            ...
            if pygame.time.get_ticks() - self.flag_place_animation_frames < 600:
                screen.blit(
                    self.flag_place_framese[
                        int(
                            10
                            * (
                                pygame.time.get_ticks()
                                - self.flag_place_animation_frames
                            )
                            / 600
                        )
                    ],
                    ...
                    self.flag_idle_frames[
                        (
                            int(
                                ...
                                * (
                                    pygame.time.get_ticks()
                                    - self.flag_idle_animation_frames
                                    - 600
                                    * int(
                                        (
                                            pygame.time.get_ticks()
        ...
        column, row = (
            ...
            (coords[1] - TOP_MARGIN) // self.cell_size,
        )
        ...
        for event in pygame.event.get():
                    ...
                    if event.button == 1 and not cell.is_flagged:
                        ...
                        if (self.start_playing and self.check_win()) or (
                            ...
                            pygame.time.set_timer(
                                REVEAL_MINES_OR_FLAGS_EVENT, int(mine_reveal_time)
                            )
                            self.stop_input = True
                            self.timer.end()
            ...
            if event.type == (REVEAL_MINES_OR_FLAGS_EVENT):
                if self.start_playing and self.check_win():
                    uncovered_mines = list(
                        filter(
                            lambda cell: cell.value == "M" and cell.flag_green == False,
                            self.cells,
                        )
                    )
                else:
                    uncovered_mines = list(
                        filter(
                            lambda cell: cell.value == "M" and cell.explode == False,
                            self.cells,
                        )
                    )
                if len(uncovered_mines) > 0:
                    ...
                    row_index, column_index = (
                        next_mine.coordinates[0],
                        next_mine.coordinates[1],
                    )
                    cell = self.cells[row_index * self.columns + column_index]

                    # If player won, change current flagged cell to a green flag and play sound effect
                    if self.start_playing and self.check_win():
                        self.music_player.play_correct_sound(
                            channel=len(uncovered_mines) + 2
                        )
                    ...
                    # If player lost, initiate the explosion animation on current mine
                        ...
                        self.music_player.play_beep_sound(
                            channel=len(uncovered_mines) + 2
                        )
                    ...
                    pygame.time.set_timer(
                        REVEAL_MINES_OR_FLAGS_EVENT,
                        int(mine_reveal_time * (len(uncovered_mines) / self.mines)),
                    )
                ...
                if self.start_playing and self.check_win():
                    save_game(
                        ...
                        self.scorer.calculate_score(
                            self.get_revealed_cells(), self.timer.get_elapsed_time()
                        ),
                        self.timer.get_elapsed_time(),
                    )
                    return "GAME_WIN", {
